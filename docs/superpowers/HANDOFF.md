...
- **Slice 3 (M4c UI) EN CURSO** (spec `2026-06-01-m4c-drilldown-interactivity-design.md`). **Hecho (en `main` `21d7432`):** `MacroTimeline` **data-driven** (8º chart, API `{macro,hoy,comps}` derivada de `phaseProfile`) cableado en el Drilldown + seeds (macroIds de 6 atletas + comp de Mara sem 16 + `SEED_VERSION` 4). **EMPEZAR ACÁ — restan:** sheets **Asignar competencia** (`setComps`) + **Añadir medalla** (`addMedal`, gated por `compite`) = primer **consumidor real** de las writes, **con manejo de error** en los submit (M4c asumía `LocalRepository` que nunca falla; contra HTTP el write puede dar 403); **overlay luteal latente** (gate `getCycleContext`, paleta neutra); **hardening `roster.ts`** (#5 índice `weeks-1` + #6 guard `Number.isFinite` de `rec`). Primitivos listos: `BottomSheet`/`Stepper`/`Toast`/`WeekPicker`.
