...
describe("cycle encryption at rest (D1)", () => {
  ...
  it("putMyCycle cifra TODOS los campos (incl. fecha/duración del ciclo) y getMyCycle los descifra", async () => {
    ...
    // Femenina: el ciclo es female-only (putMyCycle rechaza no-femeninas con CycleNotEligibleError).
    await prisma.athlete.create({ data: { id, nombre: "Enc T2", iniciales: "E2", nivel: "beginner", sexo: "F" } });
    try {
      ...
      expect(await getMyCycle(prisma, id)).toEqual({ sexo: "F", share: "full", state: "regular", lastPeriodStart: "2026-06-01", cycleLengthDays: 28, consented: true });
