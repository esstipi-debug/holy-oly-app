  ...
  return (
    <div>
      <svg viewBox={`0 0 ${SIZE} ${SIZE}`} width="100%" style={{ maxWidth: 196, display: "block", margin: "4px auto 0" }}
        role="img" aria-label={`Día ${dayInCycle + 1} de ${lengthDays}, fase ${phaseLabel(phaseToday)}`}>
        {segments.map((s) => (
        ...
        <text x={CX} y={CY + 11} textAnchor="middle" style={{ fontFamily: "var(--mono)", fontSize: 8.5, fill: "var(--wl-muted)" }}>de {lengthDays}</text>
      ...
      {nextWindow && (
        <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--wl-muted)", marginTop: 6, textAlign: "center" }}>
          Tu próximo período: <span style={{ color: "var(--wl-text)", fontWeight: 700 }}>{isoRangeLabel(nextWindow.periodStart, nextWindow.periodEnd)}</span>
        </div>
      )}
    </div>
  );
