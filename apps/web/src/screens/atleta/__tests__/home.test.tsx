...
test("atleta con plan + serie + check-in hecho: saludo con semana, estado, CTA listo", async () => {
  vi.mocked(me.getMePlan).mockResolvedValue({
    ...
    plan: { macroName: "Ruso 5D", totalWeeks: 12, currentWeek: 5, currentPhase: "Fuerza", currentPhaseKey: "fuerza", phases: [{ key: "fuerza", name: "Fuerza", from: 1, to: 12, imr: 88, imrLo: 80, imrHi: 88, volRel: 70, focus: "fuerza" }], comps: [{ name: "Nacional", week: 12 }] },
  });
...
test("usa el cliente inyectado, no el módulo global (rinde sin AthleteShell)", async () => {
  const fake: MeClient = {
    ...
    getMeHeatDays: vi.fn().mockResolvedValue({ today: "2026-01-01", weeks: [], anchorWeekIdx: 0, macroFromIdx: -1, macroToIdx: -1 }),
    getMeMacroHistory: vi.fn().mockResolvedValue({ entries: [], cyclesDone: 0, avgAdherencePct: 0 }),
    putMeSession: vi.fn(),
    anularMeSession: vi.fn(),
    desanularMeSession: vi.fn(),
    getMeCycle: vi.fn().mockResolvedValue({ share: "none", state: "regular", consented: false }),
    putMeCycle: vi.fn(),
    deleteMeCycle: vi.fn(),
  };
...
test("preview oculta la SemanaCard; sin preview la muestra", async () => {
  vi.mocked(me.getMePlan).mockResolvedValue({
    ...
    plan: { macroName: "Ruso 5D", totalWeeks: 16, currentWeek: 3, currentPhase: "Base", currentPhaseKey: "base", phases: [{ key: "base", name: "Base", from: 1, to: 16, imr: 80, imrLo: 70, imrHi: 80, volRel: 90, focus: "base" }], comps: [] },
  });
  vi.mocked(me.getMeSeries).mockResolvedValue(undefined);
