...
describe("buildWellnessRadar", () => {
  ...
  test("con entrada → 6 ejes etiquetados, hoy normalizado 0..1; SIN histórico → avg null (no inventa)", () => {
    ...
    // sin ítems semanales → NO hay promedio honesto → null (la UI dibuja sólo Hoy)
    expect(r.avg).toBeNull();
  });

  test("con TODOS los ítems semanales → avg real por ítem (por field o label)", () => {
    const series = { wellnessItems: { fatiga: [5, 5, 5], dolor: [1, 1, 1], estres: [2, 2, 2], humor: [2, 2, 2], motivacion: [4, 4, 4], sueno: [4, 4, 4] } } as unknown as MonitorSeries;
  ...
  test("histórico PARCIAL (no todos los ítems) → avg null (honesto, sin promedio a medias)", () => {
    const series = { wellnessItems: { fatiga: [5, 5], humor: [2, 2] } } as unknown as MonitorSeries;
    expect(buildWellnessRadar(entry(), series)!.avg).toBeNull();
  });
});
