...
export function CompetitionsList() {
  ...
  const proximas = items.filter((c) => c.date >= todayIso).sort((a, b) => a.date.localeCompare(b.date));
  const pasadas = items.filter((c) => c.date < todayIso).sort((a, b) => b.date.localeCompare(a.date));
  ...
  const card = (c: CompetitionListItem) => (
    <button key={c.id} type="button" onClick={() => navigate(`/coach/competencias/${c.id}`)}
      ...
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8 }}>
        ...
        <span style={{ fontFamily: "var(--mono)", fontSize: 10.5, color: c.date < todayIso ? "var(--wl-muted)" : "var(--wl-accent)", whiteSpace: "nowrap" }}>{countdownLabel(c.date, t)}</span>
      </div>
      <div style={{ marginTop: 4, fontFamily: "var(--mono)", fontSize: 11, color: "var(--wl-muted)", display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <span>{t("cardAthletes", { count: c.athleteCount })}</span>
      </div>
    </button>
  );
  ...
  return (
    <div style={{ padding: "14px 18px 26px", color: "var(--wl-text)", minHeight: "100vh", maxWidth: 390, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          <Link to="/coach" style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--wl-muted)", textDecoration: "none" }}>{t("backToRoster")}</Link>
          <h1 style={{ margin: "4px 0 0", fontFamily: "var(--wl-display)", fontWeight: 800, fontSize: 24, letterSpacing: -.4 }}>{t("listTitle")}</h1>
        </div>
      ...
      {error ? (
        ...
        <Loading style={{ fontFamily: "var(--mono)", fontSize: 11, padding: "16px 0" }}>{t("listLoading")}</Loading>
      ) : items.length === 0 ? (
        <div style={{ marginTop: 30, textAlign: "center", color: "var(--wl-muted)" }}>
          <div style={{ fontFamily: "var(--wl-display)", fontWeight: 700, fontSize: 15, color: "var(--wl-text)" }}>{t("listEmpty")}</div>
          <div style={{ fontFamily: "var(--mono)", fontSize: 11.5, marginTop: 6, lineHeight: 1.5 }}>
            {t("listEmptySub1")}<br />{t("listEmptySub2")}
          </div>
          <button type="button" onClick={() => setShowNew(true)}
        ...
        <>
          {proximas.length > 0 && (
            <>
              <SectionLabel>{t("secProximas")}</SectionLabel>
              {proximas.map(card)}
            </>
          )}
          {pasadas.length > 0 && (
            <>
              <SectionLabel>{t("secFinalizadas")}</SectionLabel>
              {pasadas.map(card)}
            </>
          )}
        </>
