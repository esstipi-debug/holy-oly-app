...
import { useRmLabels, RmEditSheet, type RmSheetMode } from "./RmEditSheet";
...
export function RmSection({ athleteId, plan, today, onRmsChange }: {
  ...
  const staleHint = t("rmStaleHint", { weeks: STALE_WEEKS });
  ...
  const vigLabel = (l: RmLift): string => {
    ...
    if (w == null) return "—";
    return w === 0 ? t("rmFixedThisWeek") : t("rmFixedWeeksAgo", { weeks: w });
  };
  ...
  return (
    <section aria-label={t("rmBaseTitle")} style={{ marginTop: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
        <div style={{ fontFamily: "var(--wl-display)", fontWeight: 700, fontSize: 13.5 }}>{t("rmBaseTitle")}</div>
        <button type="button" aria-label={t("rmEditRms")} onClick={() => setSheet({ kind: "manual" })}
        ...
        <>
            ...
            {RM_LIFTS.map((l) => {
              ...
              return (
                <div key={l} style={{ padding: "10px 12px", borderRadius: 12, background: "var(--wl-surface)", border: "1px solid color-mix(in srgb,var(--wl-text) 8%,transparent)" }}>
                  ...
                  <div title={stale ? staleHint : undefined}
          ...
          {candidates.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontFamily: "var(--wl-display)", fontWeight: 700, fontSize: 12.5 }}>{t("rmPrPending")}</div>
                ...
                <div key={c.lift} style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8, padding: "10px 12px", borderRadius: 12, background: "color-mix(in srgb,var(--wl-accent) 8%,var(--wl-surface))", border: "1px solid color-mix(in srgb,var(--wl-accent) 35%,transparent)" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--wl-muted)", marginTop: 2 }}>
                      {t("rmLifted", { kg: c.kg, when: c.doneAt ? isoDateLabel(c.doneAt) : t("compWeekLabel", { week: c.week }) })}
                    </div>
                  </div>
