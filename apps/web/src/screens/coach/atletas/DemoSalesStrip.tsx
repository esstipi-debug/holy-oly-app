      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
        {annotated.map((r) => (
          <div key={r.id} style={{ display: "flex", gap: 8, alignItems: "baseline", flexWrap: "wrap" }}>
            ...
            {r.cell === "none"
              ? <span style={{ fontFamily: "var(--mono)", fontSize: 9, color: "var(--wl-muted)", border: "1px solid color-mix(in srgb,var(--wl-text) 16%,transparent)", padding: "2px 7px", borderRadius: 99 }}>{t("readiness.none")}</span>
              : <Badge tone={r.cell}>{t(`readiness.${r.cell}`)}</Badge>}
