...
describe("LocalMeClient — ciclo (slice ciclo-visible)", () => {
  it("default honesto sin registro: share none", async () => {
    const me = new LocalMeClient("x9", new MemStorage());
    expect(await me.getMeCycle()).toEqual({ share: "none", state: "regular" });
  });

  it("roundtrip put/get + borrar campos al omitirlos", async () => {
    ...
    await me.putMeCycle(data);
    expect(await me.getMeCycle()).toEqual(data);
    await me.putMeCycle({ share: "min", state: "regular" });
    expect(await me.getMeCycle()).toEqual({ share: "min", state: "regular" });
  });

  it("input inválido rechaza (mirror del 400 del API)", async () => {
    const me = new LocalMeClient("x9", new MemStorage());
    await expect(me.putMeCycle({ share: "full", state: "regular", cycleLengthDays: 50 })).rejects.toThrow();
  });
  ...
  it("lo que registra la atleta del demo lo refleja el coach local (mismos keys)", async () => {
    ...
    await me.putMeCycle({ share: "full", state: "regular", lastPeriodStart: daysAgo(2), cycleLengthDays: 28 });
