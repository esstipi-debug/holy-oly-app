...
export function WeekDetailSheet({ open, onClose, week, dateISO, isTaper, signals, perWeek, marks, onToggle }: {
  ...
  return (
    <BottomSheet open={open} onClose={onClose} ariaLabel={t("weekDetail.ariaLabel")}>
      <div style={{ fontFamily: "var(--wl-display)", fontWeight: 800, fontSize: 16, color: "var(--wl-text)" }}>
        {t("weekDetail.week", { week })} {isTaper && <span style={{ color: STATUS.alert }}>{t("weekDetail.taper")}</span>}
      </div>
      <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--wl-muted)", marginBottom: 12 }}>{dateISO}</div>

      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        {signals.map((s) => (
          <div key={s.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, borderTop: "1px solid color-mix(in srgb,var(--wl-text) 7%,transparent)", paddingTop: 7 }}>
            <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--wl-muted)" }}>{t(`signal.${s.key}`)}</span>
            {!s.hasData
              ? <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: STATUS.none }}>{t("weekDetail.noData")}</span>
      ...
      {perWeek > 0 && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <span style={{ fontFamily: "var(--wl-display)", fontWeight: 700, fontSize: 13 }}>{t("weekDetail.adherence")}</span>
            <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--wl-muted)" }}>{t("weekDetail.adherenceHint", { done: weekDone(marks, week), perWeek })}</span>
          </div>
            ...
            {Array.from({ length: perWeek }, (_, idx) => {
              ...
              return (
                <button key={idx} type="button" onClick={() => onToggle(week, idx)} style={cell(st)}
                  aria-label={t("weekDetail.sessionAria", { week, idx: idx + 1, status: st ?? "pending" })}>
                  {st ? GLYPH[st] : ""}
                </button>
              );
            })}
