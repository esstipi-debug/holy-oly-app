...
export function AuthScreen() {
  ...
  function onGoogle(): void {
    ...
    if (mode === "signup") {
      ...
      // El sexo es obligatorio para atletas también por OAuth (así viaja en el contexto firmado).
      if (role === "atleta" && !sexo) {
        setError(t("errors.sexRequired"));
        return;
      }
      googleAuthStart({ intent: "signup", role, name: name || undefined, accept: true,
        sexo: role === "atleta" ? sexo ?? undefined : undefined,
        weightKg: role === "atleta" && weightKg.trim() ? Number(weightKg) : undefined });
    } else {
  ...
  async function onSubmit(e: FormEvent): Promise<void> {
    ...
    // El sexo es obligatorio para atletas (gatea ciclo female-only + barra 15/20 kg).
    if (mode === "signup" && role === "atleta" && !sexo) {
      setError(t("errors.sexRequired"));
      return;
    }
    setBusy(true);
      ...
      <form onSubmit={onSubmit} style={{ width: "100%", maxWidth: 360, background: "var(--wl-surface)", borderRadius: 18, padding: "22px 20px", border: "1px solid color-mix(in srgb,var(--wl-text) 8%,transparent)" }}>
        ...
        {mode === "signup" && role === "atleta" && (
          <>
            ...
            <label style={label} htmlFor="auth-weight">{t("fields.weight")}</label>
            <input id="auth-weight" style={input} type="number" inputMode="decimal" min={20} max={300} step={0.5}
