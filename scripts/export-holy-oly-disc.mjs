/** Export canonical red 25 kg disc SVG for README (port of holyOlyIconSvg.ts). */
...
function arcTop(r, span) {
  const h = span / 2;
  const [x1, y1] = pt(270 - h, r);
  const [x2, y2] = pt(270 + h, r);
  return `M${x1.toFixed(2)} ${y1.toFixed(2)} A${r} ${r} 0 0 1 ${x2.toFixed(2)} ${y2.toFixed(2)}`;
}
function arcBottom(r, span) {
  const h = span / 2;
  const [x1, y1] = pt(90 + h, r);
  const [x2, y2] = pt(90 - h, r);
  return `M${x1.toFixed(2)} ${y1.toFixed(2)} A${r} ${r} 0 0 0 ${x2.toFixed(2)} ${y2.toFixed(2)}`;
}
function defs(uid) {
  return (
    `<radialGradient id="band${uid}" cx="40%" cy="33%" r="80%"><stop offset="0" stop-color="${PAL.light}"/><stop offset="50%" stop-color="${PAL.color}"/><stop offset="100%" stop-color="${PAL.edge}"/></radialGradient>` +
...
function face(uid, withText) {
  ...
  if (withText) {
    const tp = `top${uid}`;
    const bp = `bot${uid}`;
    const common = `font-family="'Saira Condensed',Impact,sans-serif" fill="#ffffff" font-size="${CFG.fontSize}" font-weight="${CFG.weight}"`;
...
function holyOlyIconSvg(uid, withText = true) {
  const sx = 0.82;
  const sy = 0.985;
  const R = -5;
  ...
  const oy = 7;
  const wall =
    `<g transform="translate(${ox} ${oy})"><g transform="${T}"><circle cx="${CX}" cy="${CY}" r="192" fill="url(#rim${uid})"/></g></g>` +
  ...
  return (
    `<svg viewBox="0 0 ${VB} ${VB}" width="512" height="512" xmlns="http://www.w3.org/2000/svg" ` +
    `role="img" aria-label="HOLY OLY — disco 25 kg"><defs>${defs(uid)}</defs>${bg}${plate}</svg>`
  );
}

writeFileSync("_mockup/holy-oly-disc.svg", holyOlyIconSvg("readme", true), "utf8");
console.log("Wrote _mockup/holy-oly-disc.svg");
